...
function Header()
{
    ...
    $this->Cell(0,5,'Reporte Niveles',0,1,'C');
    $this->Ln(30);
...
function Footer()
{
    ...
    // Número de página
    $this->Cell(0,10,'Historial Niveles - Page '.$this->PageNo().'',0,0,'C');
    ...
    while($rs = mysql_fetch_array($id_ejecucion))
        {
            ...
            if($i%2 == 1)
            {
                ...
                $pdf->SetTextColor(0);
                $pdf->Row(array($rs['cod_nivel'], $rs['nom_nivel']));
            }
            else
            {
